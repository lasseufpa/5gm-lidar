    ...
    for sc_i, sc in enumerate(ep.scenes):
        ...
        for i in range(maxNumOfReceiversInThisEpisode):
            ...
            if validReceiversInThisScene[i]:  # pre-append
                thisString = str(numEpisode+1) + ',' + str(sc_i+1) + ',' + str(i) + ',' + rec_name_to_array_idx_map[i] + ',' + str(approximateReceiverPositions[i, 0]) + ',' + str(
                    approximateReceiverPositions[i, 1])+ ',' + str(approximateReceiverPositions[i, 2])
